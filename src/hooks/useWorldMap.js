...
// inline. Sin dependencias externas en runtime (adiós demotiles y sus nombres
// de capa frágiles). `onSelect({id, country})` abre la galería del país.
export function useWorldMap(wrapRef, onSelect) {
  useEffect(() => {
    ...
    // Fetch inmediato del GeoJSON propio (mismo origen). Si fallara, se revela
    // el fondo oscuro (on-brand) en vez de dejar el mapa oculto.
